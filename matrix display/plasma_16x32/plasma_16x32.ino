#include <PxMatrix.h>

// Pins for the matrix
#define A 2
#define B 3
#define C 4
#define D 5
#define LAT 7
#define OE 8

// Create a display object
PxMATRIX display(64, 32, LAT, OE, A, B, C, D);

// Refresh rate and color depth
#define MATRIX_FPS 30

static const int8_t PROGMEM sinetab[256] = {
     0,   2,   5,   8,  11,  15,  18,  21,
    24,  27,  30,  33,  36,  39,  42,  45,
    48,  51,  54,  56,  59,  62,  65,  67,
    70,  72,  75,  77,  80,  82,  85,  87,
    89,  91,  93,  96,  98, 100, 101, 103,
   105, 107, 108, 110, 111, 113, 114, 116,
   117, 118, 119, 120, 121, 122, 123, 123,
   124, 125, 125, 126, 126, 126, 126, 126,
   127, 126, 126, 126, 126, 126, 125, 125,
   124, 123, 123, 122, 121, 120, 119, 118,
   117, 116, 114, 113, 111, 110, 108, 107,
   105, 103, 101, 100,  98,  96,  93,  91,
    89,  87,  85,  82,  80,  77,  75,  72,
    70,  67,  65,  62,  59,  56,  54,  51,
    48,  45,  42,  39,  36,  33,  30,  27,
    24,  21,  18,  15,  11,   8,   5,   2,
     0,  -3,  -6,  -9, -12, -16, -19, -22,
   -25, -28, -31, -34, -37, -40, -43, -46,
   -49, -52, -55, -57, -60, -63, -66, -68,
   -71, -73, -76, -78, -81, -83, -86, -88,
   -90, -92, -94, -97, -99, -101, -102, -104,
  -106, -108, -109, -111, -112, -114, -115, -117,
  -118, -119, -120, -121, -122, -123, -124, -124,
  -125, -126, -126, -127, -127, -127, -127, -127,
  -128, -127, -127, -127, -127, -127, -126, -126,
  -125, -124, -124, -123, -122, -121, -120, -119,
  -118, -117, -115, -114, -112, -111, -109, -108,
  -106, -104, -102, -101,  -99,  -97,  -94,  -92,
   -90,  -88,  -86,  -83,  -81,  -78,  -76,  -73,
   -71,  -68,  -66,  -63,  -60,  -57,  -55,  -52,
   -49,  -46,  -43,  -40,  -37,  -34,  -31,  -28,
   -25,  -22,  -19,  -16,  -12,   -9,   -6,   -3
};

void setup() {
  // Initialize the display
  display.begin(4); // 1/4 scan panels
  display.setFastUpdate(true);
}

void loop() {
  static uint8_t offset = 0;
  for (uint8_t x = 0; x < 32; x++) {
    for (uint8_t y = 0; y < 16; y++) {
      int16_t color = pgm_read_byte_near(sinetab + ((x + offset) & 0xFF)) +
                      pgm_read_byte_near(sinetab + ((y + offset) & 0xFF));
      display.drawPixel(x, y, display.color565(color, color / 2, 255 - color));
    }
  }
  offset++;
  display.showBuffer();
  delay(1000 / MATRIX_FPS);
}
